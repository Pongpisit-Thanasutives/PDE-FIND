...
% burgers_learnable_ic_sim_pinn5_noiselv30_none32_prediction | 0.102793u^{1}_{xx} + -0.502842u^{2}_{x}
% in_fname = 'burgers_learnable_ic_sim_pinn5_noiselv30_none32_prediction';
% dr = ['inputs/', append(in_fname, '.mat')];

% 0.0970429u^{1}_{xx} + -0.497498u^{2}_{x}
in_fname = 'burgers_noise30';
in_fname = 'burgers_noise30_ksvdreg';
dr = ['inputs/', append(in_fname, '.mat')];

% in_fname = 'burgers';
% dr = ['/Users/pongpisit/Desktop/research/PDE-FIND/Datasets/', append(in_fname, '.mat')];
...
%     load(dr, 'usol', 'x', 't');
%     lhs = [1 0 1];
%     U_obs = cellfun(@real, {usol}, 'UniformOutput', false);
%     U_obs = cellfun(@double, U_obs, 'UniformOutput', false);
%     xs_obs = cellfun(@double, {x, t}, 'UniformOutput', false);
%     lhs = double(lhs);
end
