    ...
    for j in range(n):
        ...
        if del_[j,0] != 0:
            ...
            if dp[j,0] < 0: 
                ...
                p[j,0] = ps[j,0] - del_[j]
                J[:,j] = (y1-lm_func(t,p,func)) / (2 * del_[j,0])
                func_calls = func_calls + 1
...
def lm(p,t,y_dat,func):  
    ...
    while not stop and iteration <= MaxIter:
        ...
        if Update_Type == 2:                        
          ...
          # % residual error using p_try
          delta_y = y_dat - lm_func(t,p_try,func)     
          func_calls = func_calls + 1
          # % Chi-squared error criteria
          X2_try = delta_y.T @ ( delta_y * weight )   
...
def make_lm_plots(x,y,cvg_hst,func):
    

    # extract parameters data
    p_hst  = cvg_hst[:,2:]
    p_fit  = p_hst[-1,:]
    y_fit = lm_func(x,np.array([p_fit]).T,func)
